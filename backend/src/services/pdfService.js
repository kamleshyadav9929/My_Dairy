...
/**
 * Stream customer statement PDF directly to response
 */
function streamCustomerStatement(res, customer, entries, payments, summary, dateRange) {
...
/**
 * Stream daily collection report PDF directly to response
 */
function streamDailyReport(res, date, entries, totals, dairyName) {
    const doc = new PDFDocument({ margin: 50 });
...
// Legacy buffer-based functions for backward compatibility
function generateCustomerStatement(customer, entries, payments, summary, dateRange) {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.fontSize(20).font('Helvetica-Bold').text('Milk Passbook Statement', { align: 'center' });
            doc.moveDown();
            doc.fontSize(10).font('Helvetica').text(`Period: ${dateRange.from} to ${dateRange.to}`, { align: 'center' });
            doc.moveDown();
            ...
            doc.text('Rate', tableLeft + 200, tableTop);
            doc.text('Amount', tableLeft + 240, tableTop);
            doc.moveTo(tableLeft, tableTop + 12).lineTo(tableLeft + 290, tableTop + 12).stroke();
...
function generateDailyReport(date, entries, totals, dairyName) {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.fontSize(18).font('Helvetica-Bold').text(dairyName || 'Dairy', { align: 'center' });
            doc.fontSize(14).text('Daily Collection Report', { align: 'center' });
            ...
            doc.text('Rate', tableLeft + 265, tableTop);
            doc.text('Amount', tableLeft + 295, tableTop);
            doc.moveTo(tableLeft, tableTop + 10).lineTo(tableLeft + 340, tableTop + 10).stroke();
